...
const InputFindObject: React.FC<{
  label: string;
  modelId: string;
  value?;
  isLoading?: boolean;
  ...
  const [options, setOptions] = useState<{ value; label: string }[]>();
  ...
  useEffect(() => {
    const nl = [];
    const request = context.getObjects(modelId, {}, (response) => {
      response.data.map((r) =>
        nl.push({ value: r._id, label: r.data[primary] })
      );
      setOptions(nl);
    });
    return () => request.stop();
...
export default InputFindObject;
